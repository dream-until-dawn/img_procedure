...
def get_filename(n):
    ...
    return file_name
list_1=get_filename(6)
list_2=get_filename(5)
list_3=get_filename(4)
list_4=get_filename(3)
list_5=get_filename(2)
list_6=get_filename(1)

count_num=1
max_num=len(list_1)*len(list_2)*len(list_3)*len(list_4)*len(list_5)*len(list_6)
print('预计合成 {0} 张图片，请等待...'.format(max_num))

for i_1 in list_1:
    im_1=Image.open((flie_path+'/../../coverage_6/{0}').format(i_1))
    image_1=im_1.copy()#打开第1张

    for i_2 in list_2:
        im_2=Image.open((flie_path+'/../../coverage_5/{0}').format(i_2))
        image_2=im_2.copy()#打开第2张

        for i_3 in list_3:
            im_3=Image.open((flie_path+'/../../coverage_4/{0}').format(i_3))
            image_3=im_3.copy()#打开第3张

            for i_4 in list_4:
                im_4=Image.open((flie_path+'/../../coverage_3/{0}').format(i_4))
                image_4=im_4.copy()#打开第4张

                for i_5 in list_5:
                    im_5=Image.open((flie_path+'/../../coverage_2/{0}').format(i_5))
                    image_5=im_5.copy()#打开第5张

                    for i_6 in list_6:
                        im_6=Image.open((flie_path+'/../../coverage_1/{0}').format(i_6))
                        ...
                        #--------------------------------------------------------------------
                        image_1.save(flie_path+'/../../save/{0}{1}{2}{3}{4}{5}.png'.format(i_6[:-4],i_5[:-4],i_4[:-4],i_3[:-4],i_2[:-4],i_1[:-4]))
